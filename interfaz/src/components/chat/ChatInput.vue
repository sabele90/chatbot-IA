<template>
      ...
      <div class="chat-input-container flex gap-4 items-center p-5 bg-slate-500">
...
<div class="options flex justify-end space-x-4  bg-yellow-300">
        ...
        <div class="creativeButton">
          <button
            @click="toggleCreativityMenu"
            class="px-4 py-2 bg-indigo-600 text-white font-medium rounded-md shadow hover:bg-indigo-700 focus:outline-none"
          >
            {{ selectedCreativity || "Creatividad" }}
          </button>
          <div
            v-if="showCreativityMenu"
            class="absolute mt-2 w-30 bg-white border border-gray-300 rounded-md shadow-lg"
          >
            <ul>
              <li
                v-for="option in creativityOptions"
                :key="option"
                @click="selectCreativity(option)"
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const creativityOptions: Array<"baja" | "media" | "alta"> = ["baja", "media", "alta"];
      ...
      const sendMessage = async () => {
        if (!message.value.trim() || !selectedCreativity.value || !selectedLength.value) {
